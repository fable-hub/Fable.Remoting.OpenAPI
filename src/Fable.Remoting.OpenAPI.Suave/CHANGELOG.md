# Changelog

All notable changes to this project will be documented in this file.

The format is based on Keep a Changelog,
and this project adheres to Semantic Versioning.

## [Unreleased]

## 0.0.3 - 2026-03-17

- Updated Core version to 0.0.3 for discriminate union support and OpenAPI example serialization compatibility with Fable.Remoting.Json.

## 0.0.2 - 2026-03-16

### Changed

- Updated Core version to 0.0.2 for new example metadata features and endpoint expression support.

## 0.0.1 - 2026-03-16

### Added

- Dedicated Suave adapter package with docs endpoint webpart helpers.
- Adapter composition API to combine OpenAPI docs webpart with remoting webpart.
